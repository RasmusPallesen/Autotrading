...
def _extract_command(raw_text: str) -> str:
    """Extract the actual command from whatever format iOS Shortcuts sends."""
    text = raw_text.strip()
    # Form-encoded: message=status
    if text.startswith("message="):
        from urllib.parse import unquote_plus
        return unquote_plus(text[len("message="):]).strip()
    # Any key=value
    if "=" in text and " " not in text.split("=")[0] and len(text.split("=")[0]) < 20:
        from urllib.parse import parse_qs, unquote_plus
        try:
            parsed = parse_qs(text)
            if "message" in parsed:
                return unquote_plus(parsed["message"][0]).strip()
            if parsed:
                return unquote_plus(list(parsed.values())[0][0]).strip()
        except Exception:
            pass
    # JSON body
    if text.startswith("{"):
    ...
    try:
        ...
        for line in resp.text.strip().splitlines():
            ...
            try:
                ...
                logger.debug("Raw: %r -> cmd: %r", raw, cmd)
                if cmd:
                    messages.append(cmd)
            except Exception as e:
